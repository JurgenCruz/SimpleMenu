...
QVariant CategoryListModel::data(const QModelIndex &index, const int role) const {
    if (!index.isValid()) {
        return QVariant();
    }
    if (const int itemIndex = index.row(); itemIndex > 0) {
        return QVariant();
    }
    if (role == Qt::DisplayRole || role == DescriptionRole) {
        return m_name;
    }
    if (role == Qt::DecorationRole || role == GroupRole) {
        return QString();
    }
    if (role == IsSeparatorRole) {
        return true;
    }
    if (role == IsParentRole || role == HasChildrenRole || role == HasActionListRole) {
        return false;
    }
